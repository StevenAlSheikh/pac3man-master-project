...
def breadthFirstSearch(problem) -> object:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    priorQueue.push((start_state, [], 0), 0 + heuristic(start_state, problem))

    visited = set()

    while not priorQueue.isEmpty():
        state, path, path_cost = priorQueue.pop()
        if problem.isGoalState(state):
            return path
        ...
        if state not in visited:
            ...
            for successor, action, stepCost in problem.getSuccessors(state):
                if successor not in visited:
                    ...
                    priorQueue.push((successor, path + [action], new_cost), new_cost + heuristic(successor, problem))
    return []
    util.raiseNotDefined()
